...
import AddTransactionAgrani from '../../Components/AddTransactionAgrani';
...
function AgraniBankTran() {
    ...
    return (
        <div className='w-full h-[calc(100vh-64px)] bg-gray-100 overflow-y-scroll p-5'>
                            ...
                            <div className=' bg-white px-5 py-3 rounded shadow flex justify-between' key={transaction._id}>
                                {
                                    updateModeOpen && currentTransactionId === transaction._id ?
                                        <div>
                                            <input type="text" placeholder='Transaction Type' value={currentTransectionDetails} onChange={(e) => setCurrentTransectionDetails(e.target.value)} className='mt-5 border border-blue-500 rounded p-1 focus:outline-none w-full' />
                                            <input type="text" placeholder='Naratiive' value={currentTransectionName} onChange={(e) => setCurrentTransectionName(e.target.value)} className='mt-5 border border-blue-500 rounded p-1 focus:outline-none w-full' />
                                        ...
                                        :
                                        <div>
                                            <p className=' mb-2 text-sm'> <span className=' font-medium'>Transaction Type : </span>{transaction.transactionDetails}</p>
                                            <p className=' mb-2 text-sm'> <span className=' font-medium'>Narrative: </span>{transaction.transactionName}</p>
            ...
            {
                addTransactionMode &&
                <AddTransactionAgrani toggleAddTransactionMode={toggleAddTransactionMode} />
            }
