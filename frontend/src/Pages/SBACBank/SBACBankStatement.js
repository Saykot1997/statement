...
function SBACBankStatement() {
    ...
    const [initialBalance, setInitialBalance] = useState(700000.73);
    const [editMode, setEditMode] = useState(false);
    const [branchName, setBranchName] = useState("Moynamoti Branch");
    const [accountOpeningBranchName, setAccountOpeningBranchName] = useState("Moynamoti Branch");
    const [branchAddress, setBranchAddress] = useState("Mostafa Palace and Hazi Faruk Plaza, Nischintopur,Moynamoti Canatonment,Cum");
    const [branchPhone, setBranchPhone] = useState("(081) 73740");
    const [branchFax, setBranchFax] = useState("0");
    const [accountType, setAccountType] = useState("Saving");
    const [accountNumber, setAccountNumber] = useState("0024130000276");
    ...
    const [endStatementDate, setEndStatementDate] = useState("31-Apr-2022");
    ...
    const getBankTransactions = async (value) => {
        ...
        try {
            ...
            const res = await axios.get(`${Host}/api/user/transaction/sbac_bank`, {
                headers: {
    ...
    const statementDateChange = (option, value) => {
        ...
        if (option === "startStatementDate") {
            ...
            setStartStatementDate(GetFormatedDate(`${startStatementDateDay}/${startStatementDateMonth}/${startStatementDateYear}`, "lowercase"));
            ...
            setEndStatementDate(GetFormatedDate(`${endStatementDateDay}/${endStatementDateMonth}/${endStatementDateYear}`, "lowercase"));
            ...
            <div className=" w-full flex">
                <div className=" w-3/5 flex items-center">
                    ...
                    <div className=' ml-2 print:text-[12px]'>
                        <p className=" print:text-sm font-medium">South Bangla Agriculture & Commerce Bank Ltd.</p>
                        {
                            editMode ?
                                ...
                                :
                                <p className=" font-semibold print:font-normal my-1 print:my-0 capitalize print:text-[13px] mb-3">{branchName}</p>
                        }
                        {
                            editMode ?
                                <input type="text" placeholder='A/C opening branch' value={accountOpeningBranchName} onChange={(e) => setAccountOpeningBranchName(e.target.value)} className=' rounded p-1 my-[2px] border border-blue-500 focus:outline-none block' />
                                :
                                <p className=" font-semibold print:font-normal my-1 print:my-0 capitalize">Account Opening Branch : {accountOpeningBranchName}</p>
                        }
                        {
                            editMode ?
                                ...
                                :
                                <p className=" font-semibold print:font-normal my-1 print:my-0">{branchAddress}</p>
                        }
                                        ...
                                        <td className=" text-sm print:text-[10px] print:leading-[13px]">
                                            {/* <p>{item.date}</p> */}
                                            {
                                                editMode ?
                                                    ...
                                                    :
                                                    <span className=' capitalize'>{GetFormatedDate(item.date, "lowercase")}</span>
                                            }
                                        </td>
                                        <td className="text-sm print:text-[10px] print:leading-[13px]">
                                            <p className=' capitalize'>{GetFormatedDate(item.date, "lowercase")}</p>
                                        </td>
...
export default SBACBankStatement